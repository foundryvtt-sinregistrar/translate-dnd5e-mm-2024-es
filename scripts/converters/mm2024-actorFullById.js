 ...
 */
import { mm2024ActivitiesById } from "./mm2024-activities-by-id.js";
import { mm2024MergeEffects } from "./mm2024-merge-effects.js";
import { mm2024AdvancementById } from "./mm2024-advancement-by-id.js";

export function mm2024ActorFullById(source, translation) {
    ...
    for (const item of out) {
        ...
        if (patch.activities && item.system?.activities) {
            item.system.activities = mm2024ActivitiesById(item.system.activities, patch.activities);
        }

        // Effects
        if (patch.effects && Array.isArray(item.effects)) {
            item.effects = mm2024MergeEffects(item.effects, patch.effects);
        }

        // Advancement
        if (patch.advancement && Array.isArray(item.system?.advancement)) {
            item.system.advancement = mm2024AdvancementById(item.system.advancement, patch.advancement);
        }
    }
...
export function mm2024ActorFullById00(actor, translation) {
