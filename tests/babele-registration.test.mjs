...
for (const [language, expected] of [
    ...
    test(`registration waits for core settings, then uses configured language ${language}`, () => {
        ...
        hooks.get("babele.init")({ register(config) { assert.equal(config.module, "translate-dnd5e-mm-2024-es"); assert.equal(config.dir, "compendium"); calls.push(config.lang); } });
...
for (const language of ["es", "es-ES", "en", "en-US", "fr"]) {
    test(`converters are inactive outside Spanish: ${language}`, async () => {
        ...
        try {
            ...
            ready = true;
            hooks.get("setup")();
            assert.equal(typeof converters?.mm2024ActivitiesById, language.startsWith("es") ? "function" : "undefined");
            if (converters) {
                for (const pack of ["actors", "content", "features", "tables"]) {
                    const data = JSON.parse(readFileSync(new URL(`../compendium/dnd-monster-manual.${pack}.json`, import.meta.url)));
